...
test.describe('Webapp POC Checkout Flow', () => {
    ...
    test('ordering two items and guest checkout', async ({ page }) => {
        ...
        // Wait for the minicart side popup to appear
        await page.waitForTimeout(3000);

        // Step 4b: Cancel/close the minicart side popup
        const closeMinicartBtn = page.locator('button[aria-label="Close"], .minicart-header button, button.close, [aria-label*="close" i]').first();
        if (await closeMinicartBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
            await closeMinicartBtn.click();
        } else {
            await page.keyboard.press('Escape');
        }
        await page.waitForTimeout(1000);

        // Step 4c: Click on 'Men' category link
        const menLink = page.getByRole('link', { name: 'Men', exact: true }).first();
        await menLink.click();
        await page.waitForTimeout(2000);

        // Step 4d: Select shorts product ('KL2 Sig Short SAH - Variation GRP')
        const shortsLink = page.getByRole('link', { name: /KL2 Sig Short SAH/i }).first();
        await shortsLink.click();
        await page.waitForTimeout(2000);

        // Step 4e: Select size for shorts and Add to Bag
        const shortSize = page.getByText('XS', { exact: true })
            .or(page.getByText('S', { exact: true }))
            .or(page.getByText('M', { exact: true }));
        if (await shortSize.first().isVisible({ timeout: 5000 }).catch(() => false)) {
            await shortSize.first().evaluate(el => el.click());
        }
        ...
        const directCheckout = page.locator('button, a, [role="button"]').filter({ hasText: /^Checkout$/i }).first();
        if (await directCheckout.isVisible({ timeout: 5000 }).catch(() => false)) {
            await directCheckout.click();
        } else {
