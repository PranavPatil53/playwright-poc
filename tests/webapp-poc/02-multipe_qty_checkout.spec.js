...
test.describe('Webapp POC Checkout Flow', () => {
    ...
    test('shoe with 2 quantity checkout', async ({ page }) => {
        ...
        // Wait for the minicart drawer to open and load
        await page.waitForTimeout(3000);

        // Step 4b: Increase quantity by 1 in the minicart
        const plusBtn = page.locator('button:has-text("+"), [aria-label*="increase" i], [aria-label*="plus" i], .qty-plus, .quantity-plus, .qty-increase').first();
        if (await plusBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
            await plusBtn.click();
        } else {
            const textPlus = page.getByText('+', { exact: true }).first();
            if (await textPlus.isVisible({ timeout: 3000 }).catch(() => false)) {
                await textPlus.click();
            }
        }
        ...
        if (await directCheckout.isVisible({ timeout: 3000 }).catch(() => false)) {
            await directCheckout.click();
        } else {
            // Fallback: click minicart icon if drawer is closed, then click Checkout
